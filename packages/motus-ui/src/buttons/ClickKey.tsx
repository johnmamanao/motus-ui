...
export default function ClickKey({ onAction, disabled = false }: ClickKeyProps) {
  ...
  return (
    <motion.button
      ...
      style={{ color: '#151513' }}
      aria-label={opening ? 'Opening project' : 'Open project'}
    >
      <span className="relative min-w-0 overflow-hidden rounded-[18px] bg-[#f4f3ed]">
        ...
        <motion.span
          ...
          transition={{ duration: reduceMotion ? 0.01 : opening ? 0.42 : 0.2, ease: [0.2, 0.8, 0.2, 1] }}
          aria-hidden="true"
        />
      ...
      >
        ...
        <span className="absolute bottom-2.5 left-0 right-0 text-center font-mono text-[7px] font-semibold uppercase tracking-[0.15em] opacity-55">
          Press
        </span>
      </motion.span>
    </motion.button>
