...
export function TextMotion({
  ...
  useLayoutEffect(() => {
    ...
    if (id === 'ticker') {
      ...
      tickerCells.forEach((cell) => {
        const from =
          cell.querySelector<HTMLElement>('[data-ticker-measure-from]')?.getBoundingClientRect().width ??
          fontSize * 0.62;
        const to =
          cell.querySelector<HTMLElement>('[data-ticker-measure-to]')?.getBoundingClientRect().width ?? fontSize * 0.62;
    ...
    if (id === 'lift') {
      ...
      loop
        .to(letters, {
          y: (index) => Math.sin(index * 0.86) * 11,
          rotateZ: (index) => Math.sin(index * 0.72) * 3,
          duration: 0.48,
    ...
    if (id === 'hinge') {
      ...
      loop
        .to(letters, { rotateX: -90, yPercent: -28, opacity: 0, duration: 0.32, stagger: 0.04, ease: 'power3.in' })
        .set(letters, { rotateX: 90, yPercent: 28 })
        .to(letters, { rotateX: 0, yPercent: 0, opacity: 1, duration: 0.48, stagger: 0.04, ease: 'power4.out' });
    }
    ...
    if (id === 'drift') {
      intro.fromTo(
        letters,
        {
          x: (index) => (index % 2 ? 1 : -1) * (32 + index * 3),
          y: (index) => ((index % 3) - 1) * 42,
          rotateZ: (index) => (index % 2 ? 1 : -1) * (8 + index),
          opacity: 0,
        },
        {
          x: 0,
      ...
      loop
        .to(letters, {
          x: (index) => Math.cos(index * 0.9) * 8,
          y: (index) => Math.sin(index * 1.1) * 9,
          rotateZ: (index) => Math.sin(index) * 4,
          duration: 0.5,
    ...
    if (id === 'ripple') {
      ...
      loop
        .to(letters, {
          y: (index) => Math.sin((index - center) * 0.9) * 14,
          scale: (index) => 1 + Math.max(0, 1 - Math.abs(index - center) / 6) * 0.16,
          duration: 0.42,
    ...
    if (id === 'cascade') {
      intro.fromTo(
        letters,
        { y: (index) => 28 + index * 7, skewY: 12, opacity: 0 },
        { y: 0, skewY: 0, opacity: 1, duration: 0.72, stagger: 0.045, ease: 'power4.out' },
      );
      loop
        .to(letters, {
          y: (index) => (index - center) * 3,
          skewY: -7,
          duration: 0.42,
          stagger: 0.03,
          ease: 'power3.inOut',
        })
        .to(letters, { y: 0, skewY: 0, duration: 0.58, stagger: 0.03, ease: 'back.out(2)' });
    }

    if (id === 'magnet') {
      intro.fromTo(
        letters,
        { x: (index) => (center - index) * 24, scaleX: 0.3, opacity: 0 },
        { x: 0, scaleX: 1, opacity: 1, duration: 0.88, stagger: { each: 0.026, from: 'center' }, ease: 'expo.out' },
      );
      loop
        .to(letters, {
          x: (index) => (center - index) * 5.5,
          scaleX: 0.72,
          duration: 0.38,
    ...
    if (id === 'arc') {
      intro.fromTo(
        letters,
        {
          x: (index) => (index - center) * 13,
          y: (index) => Math.abs(index - center) * -12,
          rotateZ: (index) => (index - center) * 3.5,
          opacity: 0,
        },
        {
          x: 0,
      ...
      loop
        .to(letters, {
          y: (index) => Math.abs(index - center) * -3.2,
          rotateZ: (index) => (index - center) * 1.1,
          duration: 0.55,
          stagger: { each: 0.02, from: 'center' },
          ease: 'power3.inOut',
        })
        .to(letters, { y: 0, rotateZ: 0, duration: 0.65, stagger: { each: 0.02, from: 'center' }, ease: 'power3.out' });
    }
    ...
    if (id === 'roll') {
      ...
      loop
        .to(letters, { rotateY: 88, xPercent: 26, opacity: 0, duration: 0.3, stagger: 0.04, ease: 'power3.in' })
        .set(letters, { rotateY: -88, xPercent: -26 })
        .to(letters, { rotateY: 0, xPercent: 0, opacity: 1, duration: 0.46, stagger: 0.04, ease: 'power4.out' });
    }
    ...
    if (id === 'shutter') {
      ...
      loop
        .to(letters, { clipPath: 'inset(0 0 0 100%)', x: 10, duration: 0.34, stagger: 0.035, ease: 'power3.in' })
        .set(letters, { clipPath: 'inset(0 100% 0 0)', x: -10 })
        .to(letters, { clipPath: 'inset(0 0% 0 0)', x: 0, duration: 0.48, stagger: 0.035, ease: 'power4.out' });
    }
    ...
    if (id === 'orbit') {
      intro.fromTo(
        letters,
        {
          x: (index) => Math.cos(index * 0.86) * 74,
          y: (index) => Math.sin(index * 0.86) * 54,
          rotateZ: (index) => index * 24 - 90,
          scale: 0.42,
          opacity: 0,
        },
        {
      ...
      loop
        .to(letters, {
          x: (index) => Math.cos(index * 0.82) * 8,
          y: (index) => Math.sin(index * 0.82) * 8,
          rotateZ: (index) => Math.sin(index) * 5,
          duration: 0.62,
          stagger: { each: 0.018, from: 'center' },
          ease: 'sine.inOut',
        })
    ...
    if (id === 'weight') {
      ...
      loop
        .to(letters, { fontWeight: 820, y: -4, scaleX: 1.035, duration: 0.34, stagger: 0.055, ease: 'power3.inOut' })
        .to(letters, { fontWeight: 650, y: 0, scaleX: 1, duration: 0.52, stagger: 0.055, ease: 'power3.out' });
    }
    ...
    if (id === 'sweep') {
      ...
      loop
        .to(letters, { filter: 'blur(8px)', opacity: 0.28, y: -5, duration: 0.28, stagger: 0.045, ease: 'power2.in' })
        .to(letters, { filter: 'blur(0px)', opacity: 1, y: 0, duration: 0.46, stagger: 0.045, ease: 'power3.out' });
    }
    ...
    if (id === 'scatter') {
      intro.fromTo(
        ...
        {
          x: (index) => {
            const angle = (index / Math.max(1, letters.length - 1)) * Math.PI * 1.35 - Math.PI * 0.68;
            return Math.cos(angle) * (58 + Math.abs(index - center) * 4);
          },
          y: (index) => {
            const angle = (index / Math.max(1, letters.length - 1)) * Math.PI * 1.35 - Math.PI * 0.68;
            return Math.sin(angle) * 46;
          },
          rotateZ: (index) => (index - center) * 4.5,
          scale: 0.55,
          filter: 'blur(4px)',
          opacity: 0,
        },
      ...
      loop
        .to(letters, {
          x: (index) => (index - center) * 4.2,
          y: (index) => -Math.cos((index - center) * 0.7) * 11,
          rotateZ: (index) => (index - center) * 0.8,
          scale: 0.9,
          duration: 0.36,
          stagger: { each: 0.018, from: 'center' },
          ease: 'power3.inOut',
    ...
    if (id === 'pulse') {
      ...
      loop
        .to(letters, { scale: 1.2, y: -7, duration: 0.28, stagger: 0.045, ease: 'power3.out' })
        .to(letters, { scale: 1, y: 0, duration: 0.52, stagger: 0.045, ease: 'elastic.out(1,.58)' });
    }

    if (id === 'drop') {
      intro.fromTo(
        letters,
        { y: (index) => -90 - index * 8, rotateZ: (index) => (index % 2 ? 14 : -14), opacity: 0 },
        { y: 0, rotateZ: 0, opacity: 1, duration: 0.86, stagger: 0.045, ease: 'bounce.out' },
      );
      loop
        .to(letters, {
          y: -24,
          rotateZ: (index) => (index % 2 ? 4 : -4),
          duration: 0.34,
    ...
    if (id === 'fan') {
      intro.fromTo(
        letters,
        {
          x: (index) => (index - center) * -10,
          y: 34,
          rotateZ: (index) => (index - center) * 9,
          opacity: 0,
      ...
      loop
        .to(letters, {
          x: (index) => (index - center) * 2.4,
          y: (index) => Math.abs(index - center) * 1.7,
          rotateZ: (index) => (index - center) * 2.1,
          duration: 0.5,
          stagger: { each: 0.018, from: 'center' },
          ease: 'power3.inOut',
        })
  ...
  return (
    <div className="showcase-fullbleed relative flex min-h-[30rem] w-full items-center justify-center bg-transparent px-5 pb-5 pt-24">
      <span
        ref={root}
        data-variant={activeVariant.id}
        aria-label={
          activeVariant.id === 'ticker'
            ? `${tickerFrom} changes to ${tickerTo}. Ticker animation.`
            : `${text}. ${activeVariant.label} animation.`
        }
      ...
      >
        {activeVariant.id === 'ticker'
          ? Array.from({ length: tickerLength }, (_, index) => {
              ...
              return (
                ...
                >
                  <span data-ticker-measure-from className="invisible absolute left-0 top-0 whitespace-pre">
                    {from === ' ' ? '\u00a0' : from}
                  </span>
                  <span data-ticker-measure-to className="invisible absolute left-0 top-0 whitespace-pre">
                    {to === ' ' ? '\u00a0' : to}
                  </span>
                  <span
                    data-text-letter
          ...
          : [...text].map((letter, index) => (
              <span
                ...
                aria-hidden="true"
                key={`${letter}-${index}`}
                className={`inline-block will-change-transform ${letter === ' ' ? 'w-[.3em]' : ''}`}
              >
                {letter === ' ' ? '\u00a0' : letter}
              </span>
            ))}
